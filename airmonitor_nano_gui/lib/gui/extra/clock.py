...
# 2023-0927 PP added date on-screen
...
class Clock(DObject):
    ...
    def show(self):
        ...
        if self._dial.label is not None:
            #PP modified: v = f"{t[3]:02d}.{t[4]:02d}" if sc is None else f"{t[3]:02d}.{t[4]:02d}.{t[5]:02d}"
            d = f"{t[0]}-{t[1]:02d}-{t[2]:02d} "  # PP: add date
            v = d + (f"{t[3]:02d}.{t[4]:02d}" if sc is None else f"{t[3]:02d}.{t[4]:02d}.{t[5]:02d}")
