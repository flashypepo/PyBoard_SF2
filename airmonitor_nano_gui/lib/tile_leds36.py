...
#TODO: creat a asyncio class Tile36 for using in asyncio programs

2024-0211 PP add main as Cntrl-C does not seem te work to go from one demo to another
             modified LED36 address to 61
2023-0428 PP replace logger with logging (mp version 1.20.*)
...
class TileLED36(WBUS_DIP):
    ...
    def __init__(self, address=LED_ADDR):
        super().__init__()
        self._i2c = super().i2c()
        # 2024-0209 modified: cur_addr = self._i2c.scan()[1]  # ASSUMPTION IT IS 2nd address in scan()
        # 2024-0209: Totem rack
        #            i2c-address LED36 is at index 2
        cur_addr = self._i2c.scan()[2]  # ASSUMPTION IT IS 2nd address in scan()
        log.info(f"LED36 current I2C-address = {hex(cur_addr)} ({cur_addr})"
                  #,fn=__name__
                  )
        if address != cur_addr:
            new_addr = address
            self.set_i2caddr(cur_addr, new_addr)
            log.info(f"LED36 new I2C-address = {hex(new_addr)} ({new_addr})..."
                      #,fn=__name__
                      )
            super().toggle_power()
    ...
    def random_dots(self, dt=10):
        ...
        while True:
            ...
            self.set_dot(x, y, r, g, b)  # 2020-0504 updated
            time.sleep_ms(dt)
...
# 2024-0211 added, becasue Cntrl-C doesnot work (anymore, micropython 1.22+)
def main(demo=5):
    #  demo - execute specified demo
    assert demo in range(0, 6), "Invalid demo number {state}"
    
    try:
        # create a tile
        addr = 61  # 60: default individual LED36 address
        ...
        # Cntrl-C werkt niet meer 2024-0211 micropython 1.22+
        # demos(led_tile)
        ...
        if demo == 1:
            ...
            led_tile.brightness(128)
            led_tile.text('Micropython is sooo cool..')
            led_tile.show()  # loop
        
        elif demo == 5:
            print('random dots...')
            led_tile.brightness(10)
            led_tile.random_dots()
            
        else:
            print(f"Invalid demo ({demo})")
            pass
